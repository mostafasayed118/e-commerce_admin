...
void main() {
  ...
  testWidgets('Arabic renders the remaining count in Eastern digits',
      (WidgetTester tester) async {
    await tester.pumpWidget(wrap(
      LowStockTile(product: product(stock: 3)),
      locale: const Locale('ar'),
    ));

    expect(find.text('متبقي ٣ فقط'), findsOneWidget);
    expect(find.text('Only 3 left'), findsNothing);
  });
